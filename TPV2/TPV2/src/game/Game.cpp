...
void Game::init() {
	...
	auto* texture = &sdlutils().images().at("fighter");
	fighter_->addComponent(new ImageRenderer(texture));
	fighter_->addComponent(new FighterCtrl(5.0f, 0.2f, 3.0f));
	fighter_->addComponent(new SimpleMove());
	fighter_->addComponent(new DeAcceleration(0.995f));
	fighter_->addComponent(new ShowAtOppositeSide());
	

	fighter_->getPos().set(sdlutils().width() / 2 - texture->width() / 2, sdlutils().height() / 2 - texture->height() / 2);
	fighter_->setWidth(texture->width());
	fighter_->setHeight(texture->height());
