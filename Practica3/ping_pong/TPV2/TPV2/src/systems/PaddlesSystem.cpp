...
//	rightaddleCtrl->setType(PaddleCtrl::AI);
	...
	paddles_[0] = leftPaddle;
	paddles_[1] = rightPaddle;
...
//	}
	...
	movePaddle(paddles_[side]);
	auto tr = mngr_->getComponent<Transform>(paddles_[side]);
	netSys->sendPaddlePosition(tr);
...
void PaddlesSystem::changePaddlePos(Uint16 side, float x, float y) {
	auto tr = mngr_->getComponent<Transform>(paddles_[side]);
	tr->pos_.set(x, y);
}
